/// Cross-validation tests: swisseph.dart vs swetest (official Swiss Ephemeris CLI).
///
/// swetest is the reference test program distributed with the Swiss Ephemeris
/// C library. This test loads reference_data.json generated by
/// generate_reference.dart (which calls swetest) and compares every value
/// against swisseph.dart's FFI bindings.
///
/// Both call the same Swiss Ephemeris C code via Moshier ephemeris, so results
...
///
/// swetest outputs values with ~7 decimal places (text format limitation).
/// This limits comparison precision to ~5e-8. We use 1e-7 to allow for
/// this truncation while still catching any real FFI binding errors.
const double _eps = 1e-7;

/// Looser tolerance for values with additional computation layers
/// (rise/set iterative solver, topocentric corrections, star-based ayanamsas).
const double _epsLoose = 1e-4;

/// Tolerance for rise/set times.
///
/// Rise/set JDs are reconstructed from swetest's date/time text output
/// (HH:MM:SS.S format, ~0.1 second precision). This reconstruction
/// introduces ~1e-6 day rounding. Combined with the iterative solver
/// differences, we use 1e-3 tolerance.
const double _epsRiseSet = 1e-3;

/// Tolerance for equatorial coordinates.
///
/// swetest's -fa format outputs RA which may differ slightly from
/// swe_calc_ut with SEFLG_EQUATORIAL depending on internal conversion paths.
...
void main() {
  setUpAll(() {
    final file = File('test/swetest-validation/reference_data.json');
    if (!file.existsSync()) {
      throw StateError(
        'reference_data.json not found. Generate it first:\n'
        '  dart run test/swetest-validation/generate_reference.dart',
      );
    }
  ...
  group('julday', () {
    test('all date → JD conversions match swetest', () {
    ...
    test('all JD → date roundtrips match swetest', () {
      final entries = _ref['julday'] as List;
      for (final entry in entries) {
  ...
  group('ayanamsa values', () {
    test('all ayanamsa/date combinations match swetest', () {
      // True ayanamsas use fixed star positions internally and may show
      // small differences between different ephemeris engine states.
  ...
  group('ayanamsa names', () {
    test('all ayanamsa names match swetest', () {
  ...
  group('house cusps', () {
    test('all house system/location/date combinations match', () {
      ...
      for (final e in entries) {
        ...
        // swetest returns 12 cusps (0-indexed); our wrapper returns 13
        // (index 0 unused, C convention). Compare cusps[1..12] against
        // swetest[0..11].
        ...
        // ascmc: swetest returns 8 values, our wrapper returns 10.
        // Compare the first 8.
  ...
  group('house names', () {
    test('all house system names match swetest', () {
  ...
  group('planet names', () {
    test('all planet names match swetest', () {
  ...
  group('rise/set times', () {
    test('all rise/set events match swetest', () {
      ...
      for (final e in entries) {
        ...
        expect(result.transitTime, closeTo(expectedJd, _epsRiseSet),
  ...
  group('degnorm', () {
    test('all normalization cases match swetest reference', () {
  ...
  group('getAyanamsaExUt', () {
    test('ayanamsa values via getAyanamsaExUt match swetest', () {
      ...
      for (final entry in entries) {
        ...
        // True ayanamsas get looser tolerance
        const trueAyanamsas = {27, 28, 29, 30, 35, 36};
        final tol = trueAyanamsas.contains(sidId) ? _epsLoose : _eps;
